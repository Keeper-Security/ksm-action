...
const parseInput = (text: string): [string, string, OperationType] => {
    ...
    let selectorExec: RegExpExecArray | null
...
export class KsmAction {
    ...
    private updateRecordField(record: KeeperRecord, input: SecretsInput, value: string): void {
        ...
        // Log warnings if any, with record context
        for (const warning of validation.warnings) {
            // Strip the record UID prefix from notation -- show only the field path
            // e.g. "RPW4GSEE0gszq14Dh6O5og/field/url" -> "field/url"
            const fieldPath = input.notation.replace(/^[^/]+\//, '')
            this.logger.warning(`[${fieldPath}] ${warning}`)
        }
    ...
    async storeFileToRecord(options: SecretManagerOptions, record: KeeperRecord, source: string): Promise<void> {
        ...
        const filePath = this.validateFilePath(rawPath)

        if (!fs.existsSync(filePath)) {
            throw new Error(`File not found: ${rawPath}`)
        }

        this.logger.debug(`📖 Reading file: ${rawPath}`)
        ...
        const fileUpload: KeeperFileUpload = {
            name: fileName,
            title: fileName,
            type: this.detectMimeType(fileName),
    ...
    async run(): Promise<void> {
        try {
            ...
            // Group by record UID and process sequentially within each group
            // to avoid race conditions when multiple fields target the same record
            if (storeOps.length > 0) {
                this.logger.info(`📝 Processing ${storeOps.length} store operation(s)...`)

                const groupedByUid = new Map<string, SecretsInput[]>()
                for (const op of storeOps) {
                    const group = groupedByUid.get(op.uid) || []
                    group.push(op)
