...
describe('Safeguards', () => {
    ...
    describe('validateFieldValue', () => {
        ...
        it('should validate URL fields', () => {
            ...
            const protocolLess = validateFieldValue('url', 'www.example.com')
            expect(protocolLess.valid).toBe(true)
            expect(protocolLess.warnings.length).toBe(0)

            const relative = validateFieldValue('url', '/path/to/page')
            expect(relative.valid).toBe(true) // URLs only generate warnings
            expect(relative.warnings.length).toBeGreaterThan(0)
