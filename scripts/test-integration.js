...
async function runTests() {
    ...
    try {
        ...
        if (testRecordUid) {
            console.log('📝 Test 2: Storing value to configured test record...');
            ...
            try {
                ...
                if (error.type === 'PERMISSION_DENIED' || error.message?.includes('not editable') || error.message?.includes('access_denied')) {
                    console.error('\n❌ Permission Denied: You don\'t have write access to the test record');
                    ...
                    console.error('❌ Record not found. Check KSM_TEST_RECORD_UID in .env.local');
        ...
        if (testRecordUid) {
            ...
            try {
                ...
                if (error.type === 'PERMISSION_DENIED' || error.message?.includes('not editable') || error.message?.includes('access_denied')) {
                    console.error('❌ Permission Denied: You don\'t have write access to the test record');
                    console.error('   KSM application needs edit permissions for the shared folder.');
                    console.error('   Contact your Keeper administrator or KSM application owner.\n');
                } else {
